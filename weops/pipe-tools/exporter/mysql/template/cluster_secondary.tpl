...
kind: StatefulSet
metadata:
  name: mysql-exporter-cluster-secondary-{{VERSION}}
  namespace: mysql
spec:
  serviceName: mysql-exporter-cluster-secondary-{{VERSION}}
  replicas: 1
  selector:
    matchLabels:
      app: mysql-exporter-cluster-secondary-{{VERSION}}
  template:
    metadata:
      annotations:
    ...
    spec:
      ...
      - name: mysql-exporter-cluster-secondary-{{VERSION}}
        ...
        securityContext:
          ...
          runAsUser: 0
        args:
          - --mysqld.host=mysql-cluster-{{VERSION}}-secondary.mysql
          - --mysqld.port=3306
        env:
        - name: MYSQL_USER
          value: weops
